  ...
  const {
    ...
    mutationFn: async (value: { code: string }) => {
      ...
      router.push({
        ...
        params: { token: res.data.token, email },
      });
    },
  });
      ...
      <SafeAreaView className="flex-1 bg-bg-base">
        <View className="flex-1 justify-center max-w-sm w-full self-center px-6 py-12">
          ...
          <Text variant="body" className="text-fg-muted mb-8 text-center">
            {locale === "en"
              ? "Missing email address. Please start the password reset again."
              : "Alamat email tidak ditemukan. Silakan mulai ulang reset kata sandi."}
          </Text>
      ...
      >
        ...
        >
          <View className="flex-1 justify-center max-w-sm w-full self-center px-6 py-12">
            <View className="mb-8">
              <Text variant="title-2" className="mb-2 text-center">
                {locale === "en" ? "Enter the code" : "Masukkan kode"}
              </Text>
            ...
            <View className="gap-4">
              <Input
                label={locale === "en" ? "Reset Code" : "Kode Reset"}
                value={code}
                onChangeText={(value) => {
                  setCode(value);
                  setCodeError(
              ...
              >
                {locale === "en" ? "Verify code" : "Verifikasi kode"}
              </Button>
            </View>
